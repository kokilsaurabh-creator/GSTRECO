  ...
  for (const sap of sapRecords) {
    ...
    if (bestMatch) {
      ...
      results.push({
        ...
        doc_type: (bestMatch.invoice_type === 'C' || bestMatch.invoice_type === 'D') ? 'CDNR' : (bestMatch.invoice_type === 'AMENDMENT' ? 'B2BA' : 'B2B'),
        invoice_type: bestMatch.invoice_type || 'R',
  ...
  for (const gstr of gstrRecords) {
    if (!matchedGstrIds.has(gstr.id)) {
      results.push({
        ...
        similarity_score: 0,
        confidence_score: 0,
        doc_type: (gstr.invoice_type === 'C' || gstr.invoice_type === 'D') ? 'CDNR' : (gstr.invoice_type === 'AMENDMENT' ? 'B2BA' : 'B2B'),
        vendor_name: '-',
        invoice_num: '-',
        invoice_date: '-',
        total_tax: 0,
